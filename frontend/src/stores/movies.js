  ...
  async getMovieOmdbDataBasedOnLinkAction(payload) {
    ...
    try {
      ...
      const response = await request.get(
        `${VITE_OMDB_URL}/?apikey=${VITE_OMDB_API_KEY}&i=${imdbId[1]}`,
      )
  ...
  async getMovieOmdbDataBasedOnTitleAction(payload) {
    ...
    try {
      ...
      title = title
        ...
        .replace(/ /g, '+')
      const response = await request.get(
        `${VITE_OMDB_URL}/?apikey=${VITE_OMDB_API_KEY}&t=${title}&y=${year}`,
      )
      if (response.data.Response === 'True' && response.data.imdbRating !== 'N/A') {
  ...
  async getMovieMdbDataAction(payload) {
    ...
    try {
      if (!imdbLink) {
        ...
        const rating = await this.getMovieOmdbDataBasedOnLinkAction(payload) //call api for rating
        if (!rating) await this.getMovieImdbDataAction(payload) //scrap imdb for rating
      }
      return true
    } catch {
  ...
  setAthinoramaMovieOmdbDataAction(id, payload, imdbLink) {
    this.MOVIES.forEach((film) => {
      if (film.id === id) {
        ...
        film.imdbRating =
          payload.imdbRating && payload.imdbRating !== 'N/A' ? payload.imdbRating : 'None'
        film.popularity =
          payload.imdbVotes && payload.imdbVotes !== 'N/A' ? convertToNumber(payload.imdbVotes) : 0
...
const getters = {
  getIndividualMovie: (state) => (movieId) =>
    state.MOVIES?.find((film) => film.id === movieId) || false,
